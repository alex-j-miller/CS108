...
def sepia(image):
    ''' This function makes the image have Sepia toning '''
    ...
    for row_index in range(num_rows):
        for column_index in range(num_columns):
            rgb = image[row_index][column_index]
            image[row_index][column_index] = [
                (rgb[0] * .393) + (rgb[1] * .769) + (rgb[2] * .189),
                (rgb[0] * .349) + (rgb[1] * .686) + (rgb[2] * .168),
                (rgb[0] * .272) + (rgb[1] * .534) + (rgb[2] * .131)
                ]
    return image

def polarize(image):
    ''' This function polarizes the image '''
